...
class StepTwo extends Component {
	handleChangeForm = name => {
		return e => {
			...
			let data_restaurant = isValidateStatus(e, name, t('No Selected'), [
	...
	render() {
		...
		_.map(dataDishes.dishes, item => {
			if (
				...
				_.isEmpty(_.filter(dishFiltered, { restaurant: item.restaurant }))
			) {
			...
			<Card title={t('Step 2')} style={{ flex: 1 }}>
				<Form layout="vertical">
					<div>
						<FormItem
							{...formItemLayout}
							label={t('Please Select a Restaurant')}
							validateStatus={validation_restaurant}
							help={message_restaurant}
						>
							<Select
								value={restaurant || '---'}
								onChange={this.handleChangeForm('restaurant')}
							>
								{_.map(dishFiltered, (item, i) => {
									return (
										<Option key={`${item.restaurant}-${item.id}`}>
											{item.restaurant}
										</Option>
									)
								})}
