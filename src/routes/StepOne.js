	...
	handleChangeForm = name => {
		return e => {
			...
			if (name === 'number_people') {
				...
				let data = isValidateStatus(e, name, t('No Selected'), [
					'validation_meal',
					'message_meal',
				])
				let {
					number_people,
					validation_number_people,
					message_number_people,
				} = this.props.order.activeRecord
				payload = {
					...data,
					number_people,
					validation_number_people,
			...
			<Card title={t('Step 1')} style={{ flex: 1 }}>
				<Form layout="vertical">
					<div>
						<FormItem
							{...formItemLayout}
							label={t('Please Select a meal')}
							validateStatus={validation_meal}
							help={message_meal}
						>
					...
					<div style={{ marginTop: 20 }}>
						<FormItem
							{...formItemLayout}
							label={t('Please Enter Number of people')}
							validateStatus={validation_number_people}
							help={message_number_people}
						>
							<InputNumber
								value={number_people || 0}
								onChange={this.handleChangeForm('number_people')}
							/>
						</FormItem>
					</div>
				</Form>
